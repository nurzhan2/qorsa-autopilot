...
class Config:
    ...
    # ПРЕДОХРАНИТЕЛЬ НА ВРЕМЯ ТЕСТОВ. Пока задан — любое сообщение, адресованное
    # КЛИЕНТУ, уходит на этот id с пометкой «[тест...]», а не настоящему клиенту.
    # Владельца и менеджера не трогает: они и так внутренние. Пусто = выключено,
    # и тогда работает штатная адресация. Живёт в одной строке, потому что
    # ошибиться тут — значит написать живому клиенту во время отладки.
    test_redirect_chat = os.getenv("TEST_REDIRECT_CHAT", "").strip()
